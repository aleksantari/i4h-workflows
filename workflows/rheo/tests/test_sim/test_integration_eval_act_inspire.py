...
COMMAND = (
    "PYTHONPATH=scripts /isaac-sim/python.sh -u -m simulation.examples.eval_act_inspire "
    "--test --headless --num_episodes 1 --max_steps 2 --enable_cameras"
)
TIMEOUT = 900
TARGET_LINES = ["ACT Evaluation", "EVALUATION SUMMARY"]


class TestEvalActInspire(unittest.TestCase):
    @requires_isaac_sim
    def test_eval_act_inspire_starts_env_and_cleans_up(self):
