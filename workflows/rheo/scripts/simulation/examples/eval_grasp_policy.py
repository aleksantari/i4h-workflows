...
parser = argparse.ArgumentParser(description="Evaluate Policy on Grasp Policy Task")
parser.add_argument("--task", type=str, default="Isaac-Grasp-Policy-G129-Dex3-Joint", help="task name")
parser.add_argument(
    "--policy_type",
    type=str,
    default="gr00t",
    choices=["gr00t", "act", "test"],
    help="policy type: gr00t (GR00T VLA), act (Action Chunking Transformer), test (dummy zero policy)",
)
...
parser.add_argument("--success_stage", type=int, default=3, help="success stage for the task (grasp_policy has 3)")
parser.add_argument(
...
parser.add_argument(
    "--rl_ckpt",
    action="store_true",
    help="use RL-trained checkpoint (applies GR00T RL patch for gr00t policy type)",
)
...
def main():
    ...
    print("=" * 60)
    print("Policy Evaluation: Grasp Policy Task")
    print("=" * 60)
    ...
    if test_mode:
    ...
    elif args_cli.policy_type == "gr00t":
        ...
        # Use provided config or auto-generate a minimal one
        if args_cli.policy_config_yaml:
            config_path = args_cli.policy_config_yaml
        else:
            # Write a temporary config pointing to the model
            import tempfile
    ...
    results_file = Path("./eval_results") / f"results_{timestamp}_{args_cli.policy_type}.txt"
    ...
    if not test_mode and args_cli.policy_type == "gr00t" and bool(getattr(args_cli, "rl_ckpt", False)):
