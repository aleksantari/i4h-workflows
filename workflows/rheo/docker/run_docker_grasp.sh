...
# run_docker_grasp.sh — forked from run_docker.sh for the grasp_policy ACT pipeline.
# GR00T flags removed; uses Dockerfile.grasp (LeRobot always installed).

set -e

DOCKER_IMAGE_NAME='grasp-policy'
...
while getopts ":d:m:e:hn:rn:Rn:vn:u:N" OPTION; do
    case $OPTION in
...
if [ "$(docker images -q $DOCKER_IMAGE_NAME:$DOCKER_VERSION_TAG 2> /dev/null)" ] && \
    ...
    docker build --pull \
        ...
        --file ${WORKFLOW_DIR}/docker/Dockerfile.grasp \
        ${I4H_ROOT}
fi
